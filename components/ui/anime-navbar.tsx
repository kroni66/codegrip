...
export function AnimeNavBar({ items, className, defaultActive = "Home" }: NavBarProps) {
  ...
  const activeTabRef = useRef<string>(defaultActive)
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sections = items.map(item => item.url).filter(url => url !== '#')
      ...
      for (const sectionId of sections) {
        ...
        if (element) {
          ...
          if (scrollPosition >= elementTop && scrollPosition < elementBottom) {
            ...
            if (activeItem && activeTabRef.current !== activeItem.name) {
      ...
      if (window.scrollY < 100) {
        ...
        if (homeItem && activeTabRef.current !== homeItem.name) {
  ...
  const scrollToSection = (sectionId: string) => {
    if (sectionId === '#') {
      ...
      if (element) {
        ...
        const navbar = document.querySelector('[class*="fixed top-5"]') as HTMLElement
        const navbarHeight = navbar ? navbar.offsetHeight + 20 : 100 // Add some padding
  ...
  return (
    <div className="fixed top-5 left-0 right-0 z-[9999]">
      <div className="flex justify-center pt-6 px-4">
        <motion.div 
          className="flex items-center gap-4 bg-black/50 border border-white/10 backdrop-blur-lg py-2 px-4 rounded-full shadow-lg relative"
        ...
        >
          <img
            src="/ikonka.png"
            alt="CodeGrip Logo"
            className="w-12 h-12 hover:scale-110 transition-transform duration-300 ml-2"
          />
          <h3 className="text-xl font-bold tracking-wide text-white ml-4">
            CODEGRIP
          </h3>
          <LayoutGroup id="anime-nav">
          {items.map((item) => {
